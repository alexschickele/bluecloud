...
#' @concept computes partial dependency plot for potential and weighted patterns
#' 
...
#' @param plot_list list of enzymes to compute de PDP for, defined by their KEGG KO id.
#' @param scaled TRUE or FALSE, corresponds to taxonomically weighted or potential patterns
#' 
#' @return a pdf containing all PDPs

pdp <- function(bluecloud.wd = bluecloud_dir,
                data.wd = data_dir,
  ...
  # --- 3. Defining function for multivariate PDP adapted to MBTR
  ...
  for(i in 1:dim(X0)[2]){
    ...
    # --- 4.2. Aggregating by functions
    for(j in 1:length(plot_list)){
      # --- 4.2.1. Extract nearest neighbor data
  ...
  # --- 5. Plot between 0 and 1 all together to better compare variables
  # --- 5.1. Define the plot name and scale
  ...
  # --- 6. Plot features for comparison
