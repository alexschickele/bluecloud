...
data <- tibble(Genes =c("TARA_AON_82_MAG_00272_000000002837.1.1", "TARA_TOTO_82_MAG_00272_000000002837.1.1"))

toto <- data %>% mutate(MAG = substr(Genes, 13, 22))
toto <- data %>% mutate(MAG = sapply(strsplit(Genes, "_"), function(x) x[5]))
toto <- data %>% mutate(MAG = gsub("(.*_){2}(\\d+)_.+", "\\2", Genes))
toto
